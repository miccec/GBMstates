...
### scBiPaD
scBiPaD <- function(dataList, GO, nCores = 8, ppath = "scBipaD_output"){
  dir.create(ppath)
  ...
  for(ddat in 1:length(dataList)){
    ...
    library(doParallel)
    aMakeCluster <- makeCluster(nCores)
    ...
    for(i in 1:length(pp)){
      ...
      consensusClust <- function(ddata, G, epsilon, cpuCluster, runs = 500, bySample = TRUE, 
                                 ...
                                 saveAll = FALSE) {
        METHODS <- c("monti", "unbiased", "full")
        method <- pmatch(method, METHODS)
        if (!is.na(method)) 
          method <- METHODS[method] else stop("Invalid consensus method.")
        METHODS <- c("single", "complete", "average", "ward", "pam", 
                     "kmeans")
        clustMethod <- pmatch(clustMethod, METHODS)
        if (!is.na(clustMethod)) 
          clustMethod <- METHODS[clustMethod] else stop("Invalid clustering/linkage method.")
        METHODS <- c("euclidean", "maximum", "manhattan", "canberra")
        distMethod <- pmatch(distMethod, METHODS)
        if (!is.na(clustMethod)) 
          distMethod <- METHODS[distMethod] else stop("Invalid distance method.")
        if (bySample) {
          if (clustMethod != "kmeans" & class(ddata) == "matrix") 
            ddata <- dist(ddata, method = distMethod)
          ans <- consensus.bySample(ddata, G = G, epsilon = epsilon, 
                                    runs = runs, method = method, distMethod = distMethod, 
                                    clustMethod = clustMethod, cpuCluster = cpuCluster)
        } else ans <- consensus.byFeature(ddata, G = G, epsilon = epsilon, 
                                          runs = runs, method = method, distMethod = distMethod, 
                                          clustMethod = clustMethod, cpuCluster = cpuCluster)
        return(ans)
      }
      
      consensus.bySample <- function(ddist, G, runs, epsilon, cpuCluster, distMethod = "euclidean", 
                                     clustMethod = "ward", bySample = TRUE, method = "monti") {
        ttime <- Sys.time()
        if (!is.na(pmatch(method, "monti"))) 
          method <- "monti"
        METHODS <- c("monti", "unbiased", "full")
        method <- pmatch(method, METHODS)
        if (is.na(method)) 
          stop("invalid consensus method")
        if (clustMethod == "kmeans" & method > 1) 
          stop("the unbiased and full consensus methods are not yet implemented for k-means.")
        if (class(ddist) == "dist") 
          n <- attr(ddist, "Size") else n <- nrow(ddist)
        nn <- floor(epsilon * n)
        ssamples <- matrix(0, nrow = runs, ncol = n)
        for (h in 1:runs) ssamples[h, sample.int(n, nn, replace = FALSE)] <- 1
        if (clustMethod == "pam") {
          if (class(ddist) != "dist") 
            stop("The pam method requires an object of class dist as input")
          clusterEvalQ(cpuCluster, library(cluster))
          montiClusters <- parApply(cpuCluster, ssamples, 1, function(x, 
                                                                      nGroups, hcMeth, ddata) {
            wwhich <- which(x > 0)
            tmp <- as.matrix(ddata)[wwhich, wwhich]
            ppam <- pam(as.dist(tmp), k = nGroups)$clustering
            tmp <- rep(0, length(x))
            tmp[wwhich] <- ppam
            return(tmp)
          }, G, clustMethod, ddist)
        } else if (clustMethod == "kmeans") {
          if (class(ddist) != "matrix") 
            stop("The k-means method requires an object of class matrix as input")
          montiClusters <- parApply(cpuCluster, ssamples, 1, function(x, 
                                                                      nGroups, hcMeth, ddata) {
            wwhich <- which(x > 0)
            res <- kmeans(ddata[wwhich, ], nGroups)$cluster
            tmp <- rep(0, length(x))
            tmp[wwhich] <- res
            return(tmp)
          }, G, clustMethod, ddist)
          rownames(montiClusters) <- rownames(ddist)
        } else {
          montiClusters <- parApply(cpuCluster, ssamples, 1, function(x, 
                                                                      nGroups, hcMeth, ddata) {
            wwhich <- which(x > 0)
            tmp <- as.matrix(ddata)[wwhich, wwhich]
            hhc <- hclust(as.dist(tmp), method = hcMeth)
            tmp <- rep(0, length(x))
            tmp[wwhich] <- cutree(hhc, k = nGroups)
            return(tmp)
          }, G, clustMethod, ddist)
        }
        montiClusters <- t(montiClusters)
        cclusters <- montiClusters
        if (method > 1) {
          tmpData <- as.matrix(ddist)
          diag(tmpData) <- Inf
          notMontiResult <- parApply(cpuCluster, montiClusters, 
                                     1, function(cclust, ddata) {
                                       unclassified <- which(cclust == 0)
                                       names(cclust) <- colnames(ddata)
                                       for (entry in unclassified) {
                                         tmp <- names(which.min(ddata[entry, -unclassified]))
                                         cclust[entry] <- cclust[tmp]
                                       }
                                       return(cclust)
                                     }, tmpData)
          notMontiResult <- t(notMontiResult)
          cclusters <- notMontiResult
        }
        if (method == 2) {
          tmp <- which(montiClusters > 0, arr.ind = TRUE)
          cclusters <- notMontiResult
          cclusters[tmp] <- 0
        }
        if (method == 3) 
        ...
        dyn.load("cconsensus.so")
        ans <- .Fortran("cconsensus", cclusters, n, H, tmp, m)[[4]]
        if (method < 3) {
          if (method == 2) 
            ssamples <- 0 + ssamples < 1
          tmp <- rep(0, m)
          storage.mode(ssamples) <- "integer"
          storage.mode(n) <- "integer"
          storage.mode(H) <- "integer"
          storage.mode(m) <- "integer"
          storage.mode(tmp) <- "integer"
          dyn.load("cconsensus.so")
          tmp <- .Fortran("cconsensus", ssamples, n, H, tmp, m)[[4]]
          ddiss <- 1 - ans/tmp
        } else {
          ddiss <- 1 - ans/runs
        }
        attr(ddiss, "Size") <- n
        attr(ddiss, "Labels") <- attr(ddist, "Labels")
        attr(ddiss, "Diag") <- FALSE
        attr(ddiss, "method") <- "consensus"
        class(ddiss) <- "dist"
        ttime <- Sys.time() - ttime
        ans <- list()
        ans$inputDist <- ddist
        ans$distMethod <- attr(ddist, "method")
        ans$clustMethod <- clustMethod
        ans$consensusMethod <- METHODS[method]
        ans$bySample <- TRUE
        ans$G <- G
        ans$epsilon <- epsilon
        ans$subsetDimension <- nn
        ans$runs <- runs
        ans$clusters <- cclusters
        colnames(ans$clusters) <- attr(ddist, "Labels")
        ans$consensus.diss <- ddiss
        ans$elapsedTime <- ttime
        ans$nCpuCluster <- length(cpuCluster)
        class(ans) <- "consensus"
        return(ans)
      }
      ...
      consMatrix <- as.dist(as.matrix(aConsensus$consensus.diss))
  ...
  aConsensus <- consensusClust(as.dist(jaccard),
                               G = 4,
                               runs = 10000,
                               epsilon = .7,
                               clustMethod = "ward",
